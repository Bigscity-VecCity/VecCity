...
def generate_rel():
    rel_path = os.path.join(dataset, f'{dataset}.rel')
    if os.path.exists(os.path.join(dataset, 'roadsegment.rel')):
        rel_path = os.path.join(dataset, 'roadsegment.rel')
    elif os.path.exists(os.path.join(dataset, f'{dataset}_roadmap_edge.rel')):
        rel_path = os.path.join(dataset, f'{dataset}_roadmap_edge.rel')
    ...
    road_rel_df.to_csv(os.path.join(dataset, f'{dataset}.rel'), index=False)
...
def generate_poi2region():
    ...
    rel_df = pd.read_csv(os.path.join(dataset, f'{dataset}.rel'))
    new_rows = []
    rel_id = len(rel_df)
    SWAP = True
    ...
    rel_df.to_csv(os.path.join(dataset, f'{dataset}.rel'), index=False)

def generate_od():
    df = pd.read_csv(os.path.join(dataset, f'{dataset}.od'))
    df['flow'] = [1] * len(df)
    df.to_csv(os.path.join(dataset, f'{dataset}.od'), index=False)
...
def generate_map_match():
    ...
    if not os.path.exists(os.path.join(f'{dataset}_mm', f'{dataset}.rel')):
        org_path = os.path.join(f'{dataset}', f'{dataset}.rel')
        new_path = os.path.join(f'{dataset}_mm', f'{dataset}.rel')
...
def generate_od_from_points():
    ...
    df['flow'] = [1] * len(df)
    df.to_csv(f'{dataset}/{dataset}.od', index=False)
...
def generate_od_from_road_traj():
    ...
    rel_df = pd.read_csv(f'{dataset}/{dataset}.rel')
...
if __name__ == '__main__':
    ...
    # add_road_geometry()
    # generate_checkin()
    # generate_od_from_road_traj()
    # modify_property('road_lanes')
    # str2line()
    # add_road_features()
    # modify_highway()
    # def mp(x):
    #     return x - 207
    # mp_col('region', 'type', mp)
    # add_offset()
    def mp(x):
        print(x)
        if isinstance(x, str):
