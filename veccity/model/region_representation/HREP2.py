...
class HREABL(nn.Module):
    def __init__(self, embedding_size, dropout, gcn_layers):
        super(HREABL, self).__init__()
    ...
    def forward(self, features, rel_emb, edge_index, is_training=True):
        ...
        n_f = torch.multiply(n_emb, n_r)
        poi_f = torch.multiply(poi_emb, poi_r)
        s_f = torch.multiply(s_emb, s_r)
        d_f = torch.multiply(d_emb, d_r)

        # return region_feature, n_f, poi_f, s_f, d_f
        return n_emb, n_f, poi_f, s_f, d_f
...
class HREP(AbstractReprLearningModel):
    ...
    def __init__(self, config, data_feature):
        ...
        self.inflow_labels=torch.from_numpy(data_feature.get('inflow_labels')).float().unsqueeze(-1).to(self.device)
        self.outflow_labels=torch.from_numpy(data_feature.get('outflow_labels')).float().unsqueeze(-1).to(self.device)
        self.infer_loss_fn=nn.MSELoss()
        self.ablation=config.get('abl','none')
        self.inflow_mlp = nn.Sequential(nn.Linear(self.embedding_size,self.embedding_size),nn.ReLU(),nn.Linear(self.embedding_size, 1)).to(self.device)
        self.outflow_mlp = nn.Sequential(nn.Linear(self.embedding_size,self.embedding_size),nn.ReLU(),nn.Linear(self.embedding_size, 1)).to(self.device)
        if self.ablation == 'gcn':
            self.net = HREABL(self.embedding_size, self.dropout, self.gcn_layers).to(self.device)
        else:
    ...
    def infer_loss(self,region_emb):
        in_flow_preds=self.inflow_mlp(region_emb)
        out_flow_preds=self.outflow_mlp(region_emb)

        inflow_loss=self.infer_loss_fn(in_flow_preds,self.inflow_labels)
        outflow_loss=self.infer_loss_fn(out_flow_preds,self.outflow_labels)
        loss=(inflow_loss+outflow_loss)/2.0
        return loss
    ...
    def run(self, data=None,eval_data=None):
        ...
        for epoch in range(self.epochs):
            ...
            if self.ablation=='recon':
                loss=geo_loss
            if self.ablation=='contr':
                loss=poi_loss+m_loss
            if self.ablation=='infer':
                loss+=self.infer_loss(region_emb)
            if self.ablation=='recon&infer':
                loss=geo_loss+self.infer_loss(region_emb)
            if self.ablation=='contr&infer':
                loss=poi_loss+m_loss+self.infer_loss(region_emb)
            loss.backward()
            optimizer.step()
