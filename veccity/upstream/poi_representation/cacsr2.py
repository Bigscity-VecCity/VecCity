...
class CACSR(AbstractModel):
    def __init__(self, config, data_feature):
        ...
        self.encoder_layer=nn.TransformerEncoderLayer(self.loc_emb_size,4,self.hidden_size)
        self.lstm = nn.TransformerEncoder(self.encoder_layer,self.num_layers,norm=nn.LayerNorm(self.loc_emb_size, eps=1e-6))
    ...
    def forward(self, batch, mode='test', adv=1):
        ...
        if mode == 'train' and adv == 1:
            ...
            x_STNPos = (loc_emb_STNPos+tim_emb_STNPos).permute(1, 0, 2)
            
            src_key_mask= loc == (self.loc_size-1)
            src_mask = None
            lstm_out_STNPos=self.lstm(x_STNPos, mask=src_mask,
                                src_key_padding_mask=src_key_mask)
            lstm_out_STNPos=lstm_out_STNPos.permute(1, 0, 2)
            
            
            final_out_STNPos = lstm_out_STNPos[:, 0, :]

        # concatenate and permute
        # x = torch.cat([loc_emb, tim_emb], dim=2).permute(1, 0, 2)  # batch_first=False
        x=(loc_emb+tim_emb).permute(1, 0, 2)
        

        src_key_mask= loc == (self.loc_size-1)
        src_mask = None
        lstm_out = self.lstm(x, mask=src_mask,src_key_padding_mask=src_key_mask)
        lstm_out = lstm_out.permute(1, 0, 2)

        final_out = lstm_out[:,0,:]
        dense=self.dense(final_out)
    ...
    def encode(self, inputs):
        ...
        src_key_mask= loc == (self.loc_size-1)
        src_mask = None
        lstm_out = self.lstm(x, mask=src_mask,src_key_padding_mask=src_key_mask)  # max_len*batch*hidden_size
        lstm_out=lstm_out.permute(1, 0, 2)
        return lstm_out
