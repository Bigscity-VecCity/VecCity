...
class CTLEDataLoader(POIRepresentationDataLoader):
    ...
    def next_batch(self):
        ...
        for batch in next_batch(sklearn.utils.shuffle(list(zip(src_tokens, src_weekdays, src_ts, src_lens,user_ids))),
                                batch_size=self.batch_size):
            # Value filled with num_loc stands for masked tokens that shouldn't be considered.
            src_batch, _, src_t_batch, src_len_batch,user_id = zip(*batch)
            user_id=torch.tensor(user_id).long().to(self.device)
            ...
            yield origin_tokens, origin_hour, masked_tokens, src_t_batch, mask_index,src_len,user_id,src_seq
